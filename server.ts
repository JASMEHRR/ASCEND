...
async function startServer() {
  ...
  app.get("/api/auth/google/url", (req, res) => {
    try {
      ...
      // 2. Pass the exact redirectUri into the state so we can recover it in the callback
  ...
  app.get("/api/auth/callback/google", async (req, res) => {
    ...
    try {
      ...
      // Use the exact redirectUri from the state for safety or fallback to the dynamic correct one
      ...
      // We send the access_token back to the client via postMessage, but also store securely in an HTTPOnly cookie
        ...
        <html>
          ...
          <body>
            ...
            <script>
              ...
              if (window.opener) {
                // Send tokens back to main page context
  ...
  app.get("/api/steps", async (req, res) => {
    try {
      ...
      // Determine local timezone timestamps from client or fallback to server UTC
      const now = new Date();
      ...
      // Initialize oauth client properly
      ...
      if (duration <= 0) {
        duration = 86400000; // default 1 day if clock skew
      }
        ...
        try {
          ...
          const res = await fitness.users.dataset.aggregate({
            ...
            requestBody: {
              ...
              bucketByTime: { durationMillis: "3600000" }, // 1-hour buckets prevent boundary edge-clipping
          ...
          if (errMsg.includes('credentials') || errMsg.includes('client') || errMsg.includes('key') || errMsg.includes('invalid_grant') || err.code === 401) {
            throw err; // Bubble up auth errors so that they are handled uniformly as authorization failures
          }
          return 0;
        }
      };
      ...
      // Return a 401 if it's an authentication or client/credentials issue
