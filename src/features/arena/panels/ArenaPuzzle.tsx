/**
 * The Puzzle tab: your own picture, spare tiles, older windows you can still
 * repair — plus, in a 'shared' room, that room's group game (lobby or the
 * live shared canvas), which runs independently and never pauses your own.
 */
...
export default function ArenaPuzzle({ onNeedHabits }: { onNeedHabits?: () => void }) {
  const { habits, players, room, today } = useArena();
  const {
    loading,
    busy,
    current,
  ...
  const isSharedRoom = room?.puzzleMode === 'shared';
  const myGameCanvas = room ? gameCanvases[room.id] : undefined;
  ...
  const applyUrl = async (src: string) => {
    ...
    try {
      ...
      if (current) await changeImage(src, aspect);
      else await setImage(src, aspect, today, 7);
      setChanging(false);
    } catch (e) {
  ...
  const onFile = async (file: File) => {
    ...
    try {
      ...
      if (current) await changeImage(dataUrl, aspect);
      else await setImage(dataUrl, aspect, today, 7);
      setChanging(false);
    } catch (e) {
  ...
  const placeOn = async (id: string) => {
    const tile = await place(id);
    if (!tile) return;
    setLanding(tile.index);
    window.setTimeout(() => setLanding(null), 700);
  };

  const placeOnRoom = async () => {
    if (!room) return;
    const tile = await placeInRoom(room.id);
    if (!tile) return;
    setLanding(tile.index);
    window.setTimeout(() => setLanding(null), 700);
  };

  // The room game section: only for shared rooms, shown above the personal
  // puzzle since it's a separate, parallel track — never a replacement.
  const roomGameSection = isSharedRoom && (
    <section className="space-y-3 rounded-3xl border border-white/8 bg-white/[0.02] p-4">
      <p className="font-mono text-[9.5px] font-bold uppercase tracking-widest text-white/40">
        {room!.name} · group game
      </p>
      {room!.game?.status === 'active' && myGameCanvas ? (
        <div className="space-y-3">
          <div className="flex items-center gap-3">
            <span className="min-w-0 flex-1">
              <p className="text-[13px] font-bold text-white/90">
                {myGameCanvas.placed} / {myGameCanvas.total} pieces
              </p>
              <p className="font-mono text-[10px] text-white/35">
                {myGameCanvas.complete ? 'Finished' : `${myGameCanvas.total - myGameCanvas.placed} to go`} · ends{' '}
                {room!.game.endsAt ? new Date(room!.game.endsAt).toLocaleDateString() : ''}
              </p>
            </span>
            {availableForRoom(room!.id) > 0 && !myGameCanvas.complete && (
              <button
                onClick={() => void placeOnRoom()}
                disabled={busy}
              ...
              >
                <Puzzle size={13} /> Place ({availableForRoom(room!.id)})
              </button>
            )}
          </div>
          <PuzzleCanvas
            imageUrl={myGameCanvas.imageUrl}
            cols={myGameCanvas.cols}
            rows={myGameCanvas.rows}
            aspect={myGameCanvas.aspect}
            placements={myGameCanvas.placements}
  ...
  // No picture yet — or the player asked to swap the current one.
  if (!current || changing) {
    return (
      <div className="space-y-4">
        {roomGameSection}
        <div className="mx-auto max-w-md space-y-4 py-6">
          <div className="text-center">
            ...
            <h3 className="mt-3 text-[15px] font-bold text-white">Pick your picture</h3>
            <p className="mt-1 text-[12px] leading-snug text-white/40">
              {habits.length} {habits.length === 1 ? 'habit' : 'habits'} × 7 days ={' '}
              <span className="font-bold text-white/60">{habits.length * 7} pieces</span> for a week-long puzzle.
            </p>
          </div>
    ...
    <div className="space-y-4">
      ...
      <div className="flex items-center gap-3">
        <span className="min-w-0 flex-1">
          ...
          <p className="font-mono text-[10px] text-white/35">
            {current.complete ? 'Finished' : `${current.total - current.placed} to go`} · {current.id}
          </p>
        </span>
        {available > 0 && !current.complete && (
          <button
            onClick={() => placeOn(current.id)}
            disabled={busy}
            className="flex shrink-0 items-center gap-1.5 rounded-full bg-brand-500 px-3.5 py-2 text-[11px] font-bold text-black transition-all hover:bg-brand-400 disabled:opacity-40 cursor-pointer"
          >
      ...
      {/* Old unfinished windows — spare tiles can still rescue them. */}
      {repairable.length > 0 && (
        <section className="space-y-2 border-t border-white/8 pt-4">
          <p className="flex items-center gap-1.5 font-mono text-[9.5px] font-bold uppercase tracking-widest text-white/40">
            <Wrench size={11} /> Unfinished pictures
          </p>
          ...
          <div className="grid grid-cols-2 gap-2.5 sm:grid-cols-3">
            {repairable.map((c) => (
              <button
                key={c.id}
                onClick={() => available > 0 && placeOn(c.id)}
              ...
              >
                <PuzzleCanvas imageUrl={c.imageUrl} cols={c.cols} rows={c.rows} aspect={c.aspect} placements={c.placements} />
                <span className="block px-0.5 font-mono text-[9.5px] text-white/35">
                  {c.id} · {c.placed}/{c.total}
                </span>
              </button>
            ))}
          </div>
            ...
            {canvases
              ...
              .map((c) => (
                <div key={c.id} className="space-y-1">
                  <PuzzleCanvas imageUrl={c.imageUrl} cols={c.cols} rows={c.rows} aspect={c.aspect} placements={c.placements} />
                  <span className="block px-0.5 font-mono text-[9px] text-white/30">{c.id}</span>
                </div>
              ))}
          </div>
        </section>
