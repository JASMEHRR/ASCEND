...
import { Loader2, Unplug, CheckCircle2, MessageSquareText } from 'lucide-react';
...
export default function GoogleSettings() {
  ...
  return (
    <section className="space-y-2">
      ...
      {/* Separate from the connect button above on purpose: that one grants a
          browser-only token that dies when the tab closes, which can't power
          a server cron. This is a one-time redirect to Google's server-side
          consent screen — see google-oauth-routes.ts — done once, not per
          session. */}
      <div className="border-t border-white/8 pt-3">
        <a
          href="/api/google-oauth/start"
          className="flex w-full items-center justify-center gap-2 rounded-2xl border border-white/10 bg-white/[0.04] px-3.5 py-2.5 text-[12px] font-bold text-white/85 transition-all hover:border-white/20 hover:bg-white/[0.09]"
        >
          <MessageSquareText size={14} className="text-brand-400" />
          Connect Calendar + Gmail for Telegram alerts
        </a>
        <p className="mt-1.5 text-[10.5px] leading-snug text-white/30">
          One-time setup so Jarvis can text you before events and when important email arrives, even when Ascend isn't
          open. Connected before email alerts existed? Connect again and tick the Gmail box. Needs the server configured
          first, see GOOGLE_SETUP.md.
        </p>
      </div>
    </section>
  );
